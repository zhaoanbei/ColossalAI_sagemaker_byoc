# From hpcaitech/colossalai:0.2.0
...
COPY requirements.txt.1 /opt/ml/
COPY setup.py /opt/ml/
RUN pip install -r requirements.txt.1 && pip install lightning && pip install -e . && pip install boto3
